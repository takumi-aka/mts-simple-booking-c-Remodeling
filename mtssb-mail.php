...
class MTSSB_Mail {
	...
	private function _booking_content($booking, $template) {
		...
		$body = apply_filters('booking_form_number_title', '[ご予約]', 'mail') . "\n"
		 . "{$article['name']}\n"
		 . apply_filters('booking_form_date_title', '日時：') . apply_filters('booking_form_date', date('Y年n月j日', $booking['booking_time']), $booking['booking_time']) . $booking['visit_time'] . "\n"
		...
		if (!empty($booking['note'])) {
			...
			$body .= 'ご紹介者のお名前' . "：{$booking[introducer]}\n";
