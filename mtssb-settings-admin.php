...
<?php
	...
	public function add_fields_settings() {
		...
		if ($this->tab == 'controls') {
			...
			add_settings_field('opening_hours', __('開店時刻', $this->domain), array($this, 'controls_form'),
				$option_name, $option_name, array('label_for' => 'opening_hours'));
			add_settings_field('last_reserved_time', __('最終予約時刻', $this->domain), array($this, 'controls_form'),
				$option_name, $option_name, array('label_for' => 'last_reserved_time'));
	...
	public function controls_form($args) {
		...
		switch ($args['label_for']) {
			...
			case 'message' : ?>
				...
				break;
			case 'opening_hours' :
					$accepting = $this->_opening_hours_margin(); ?>
					<select id="opening_hours" name="mts_simple_booking_controls[opening_hours]" style="letter-spacing:2px"><?php foreach ($accepting as $min => $label) : ?>
						<option value="<?php echo $min ?>"<?php echo $min == $this->data['opening_hours'] ? ' selected="selected"' : '' ?>><?php echo $label ?></option>
					<?php endforeach; ?></select> <?php _e('after', $this->domain) ?><br />
					<?php _e('BAR 織瑠の開店時刻を入力します。', $this->domain);
				break;
			case 'last_reserved_time' :
					$accepting = $this->_last_reserved_time_margin(); ?>
					<select id="last_reserved_time" name="mts_simple_booking_controls[last_reserved_time]" style="letter-spacing:2px"><?php foreach ($accepting as $min => $label) : ?>
						<option value="<?php echo $min ?>"<?php echo $min == $this->data['last_reserved_time'] ? ' selected="selected"' : '' ?>><?php echo $label ?></option>
					<?php endforeach; ?></select> <?php _e('after', $this->domain) ?><br />
					<?php _e('BAR 織瑠の最終予約可能時刻を入力します。', $this->domain);
				break;
			default :
				break;
		}
	...
	private function _get_default($tab) {
		...
		$options = array(
			'controls' => array(
				...
				'opening_hours' => 7,	// 開店時刻
				'last_reserved_time' => 13,	// 最終予約可能時刻
			),
	 ...
	 * 開店時間のマージン
	 */
	private function _opening_hours_margin()
	{
		return apply_filters('mtssb_settings_opening_hours_margin', array(
			'0' => __('16:30', $this->domain),
			'1' => __('17:00', $this->domain),
			'2' => __('17:30', $this->domain),
			'3' => __('18:00', $this->domain),
			'4' => __('18:30', $this->domain),
			'5' => __('19:00', $this->domain),
			'6' => __('19:30', $this->domain),
			'7' => __('20:00', $this->domain),
			'8' => __('20:30', $this->domain),
			'9' => __('21:00', $this->domain),
			'10' => __('21:30', $this->domain),
			'11' => __('22:00', $this->domain),
			'12' => __('22:30', $this->domain),
			'13' => __('23:00', $this->domain),
			'14' => __('23:30', $this->domain),
			'15' => __('24:00', $this->domain),
		));
	}
	 ...
	 * 最終予約可能時間のマージン
	 */
	private function _last_reserved_time_margin()
	{
		return apply_filters('mtssb_settings_last_reserved_time_margin', array(
			'0' => __('16:30', $this->domain),
			'1' => __('17:00', $this->domain),
			'2' => __('17:30', $this->domain),
			'3' => __('18:00', $this->domain),
