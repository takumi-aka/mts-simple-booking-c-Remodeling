...
class MTSSB_Booking {
	 ...
	 * @daytime
	 */
	public function get_booking_of_name($daytime, $article_ids='') {
		global $wpdb;
		...
		$cal = $wpdb->get_results($wpdb->prepare("
			SELECT booking_id,booking_time,article_id,show_name,client
			FROM $this->tblBooking
		...
		foreach ($cal as $key => $booking) {
			if(strcmp($booking['show_name'],"表示しない")==0){
			}
			else{
				$client_top = unserialize($booking['client']);
				if(strlen($client_top['name'])>0){
					$cal[$daytime]['cl_name'] = $cal[$daytime]['cl_name'] . "<br>" . $client_top['name'];
				}
				else{
					$cal[$daytime]['cl_name'] = $client_top['name'];
				}
			}
		}
	...
	public function add_booking() {
		...
		$result = $wpdb->insert($this->tblBooking, $this->record,
			array('%d', '%d', '%d', '%d', '%d', '%d', '%d', '%s', '%s', '%s', '%s','%s', '%s', '%s'));
	...
	public function save_booking() {
		...
		$result = $wpdb->update($this->tblBooking, $this->record, $where,
			array('%d', '%d', '%d', '%d', '%d', '%d', '%s', '%s', '%s', '%s', '%s', '%s', '%s'), array('%d'));
	...
	public function normalize_booking($post, $count=array()) {
		...
		// 追加されたお客様情報
		$booking['visit_time'] = strval($post['visit_time']);

		$booking['show_name'] = strval($post['show_name']);

		$booking['introducer'] = strval($post['introducer']);
	 ...
	 */
	 protected function _recordData() {
		...
		$record['options'] = null;
		$record['visit_time'] = $this->booking['visit_time'];
		$record['show_name'] = $this->booking['show_name'];
		$record['introducer'] = $this->booking['introducer'];
	...
	private function _install_table() {
		...
		if (empty($version) || $version != self::TABLE_VERSION) {
			...
			$sql = "CREATE TABLE " . $this->tblBooking . " (
				...
				options text,
				visit_time char(5) DEFAULT NULL,
				show_name char(10) DEFAULT NULL,
				introducer char(40) DEFAULT NULL,
				client text,
				note text,
