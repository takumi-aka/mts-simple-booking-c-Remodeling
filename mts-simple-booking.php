<?php
/*
Plugin Name: MTS Simple Booking-C 改造
...
*/
/*
 * Updated to 2.0.0 on 2021-11-29
...
class MTS_Simple_Booking {
	...
	public function init_mtssb()
	{
		...
		// ショートコードの登録　※名前表示用のカレンダー
			add_shortcode('name_calendar', array($this, 'name_calendar'));
	...
	/**
	 * ショートコード 名前表示カレンダーのロード・実行
	 *
	 */
	public function name_calendar($atts) {
		if (!class_exists('MTSSB_Front')) {
			require_once(dirname(__FILE__) . '/mtssb-top-calendar.php');
		}

		$this->oFront = new MTSSB_Front();
		return $this->oFront->name_calendar($atts);
	}
