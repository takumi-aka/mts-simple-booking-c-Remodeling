...
class MTSSB_Booking_Form extends MTSSB_Booking {
	...
	public function __construct() {
		...
		//開店時間、最終予約時間の取得
		$this->controls['opening_hours'];
		$this->controls['last_reserved_time'];
	}
...
<form method="post" action="<?php echo $url ?>">
	...
	<table>
		...
		<tr>
			<th><label>ご来店予定時刻</label></th>
			<td>
					<?php $accepting = $this->reservation_time_margin();  //ここで時間リストの取り込み
					$slice_accepting = array_slice($accepting, $this->controls['opening_hours'], $this->controls['last_reserved_time']-$this->controls['opening_hours']+1); //表示させる時間の間隔を設定 ?>
					<select  type="text" name="booking[visit_time]" class="content-text medium"  required><?php foreach ($slice_accepting as $min => $label) : ?>
		...
		<tr>
			...
			<td>
			<?php $this->reserved = $this->get_reserved_day_count($this->thetime - $this->thetime % 86400) ;
					if (isset($this->reserved[$this->thetime][$this->article_id])) 
						$reserved = &$this->reserved[$this->thetime][$this->article_id]; ?>
			<select type="text" class="content-text small right" name="booking[client][adult]" required>
			<?php for($i = 1 ; $i <= (($this->article['capacity'] + intval($this->schedule['delta'])) - $reserved['number']) ; ++$i){ ?><option><?php echo $i ?></option><?php } ?>
			</select>	
			</td>
		</tr>
	</table>
...
<form method="post" action="<?php echo $url ?>">
	...
	<table>
		...
		<tr>
			<th>ご来店予定時刻</th>
			<td><?php echo esc_html($this->booking['visit_time']) ?><br />
					<input type="hidden" name="booking[visit_time]" value="<?php echo esc_html($this->booking['visit_time']) ?>" />
			</td>
		</tr>
		<tr>
			<th>予約人数</th>
		...
		<tr>
			<th>ご紹介者のお名前</th>
			<td>
				<?php echo esc_html($this->booking['introducer']) ?>
				<input type="hidden" name="booking[introducer]" value="<?php echo esc_html($this->booking['introducer']) ?>" />
			</td>
		</tr>
		...
		<tr>
			<th>カレンダーに名前の表示</th>
			<td>
				<?php echo esc_html($this->booking['show_name']) ?>
				<input type="hidden" name="booking[show_name]" value="<?php echo esc_html($this->booking['show_name']) ?>" />
			</td>
		</tr>
		<tr>
			<th>内容</th>
