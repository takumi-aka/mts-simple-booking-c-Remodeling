...
class MTSSB_Booking_Admin extends MTSSB_Booking {
	 ...
	 * 予約時間のマージン
	 */
	private function reservation_time_margin()
	{
		return apply_filters('reservation_time_margin', array(
			'0' => ('16:30'),
			'1' => ('17:00'),
			'2' => ('17:30'),
			'3' => ('18:00'),
			...
			<div id="poststuff">
				<div id="post-body" class="metabox-holder columns-2">
					...
					<div id="" class="postbox-container">
						<?php $this->_postbox_visit_time() ?>
						<?php $this->_postbox_show_name() ?>
						<?php $this->_postbox_show_introducer() ?>
					</div>
		...
		<div class="inside">
			<table class="form-table" style="width: 100%">
				...
				<tr>
					...
					<td>
						大人<input id="booking-adult" class="small-text" type="text" name="booking[client][adult]" value="<?php echo esc_attr($client['adult']) ?>" />人<br>
						子供<input class="small-text" type="text" name="booking[client][child]" value="<?php echo esc_attr($client['child']) ?>" />人<br>
						幼児<input class="small-text" type="text" name="booking[client][baby]" value="<?php echo esc_attr($client['baby']) ?>" />人<br>
...
<?php
	 ...
	 *
	 */
	private function _postbox_visit_time() {
				...
				<div class="inside">
					<table class="form-table" style="width: 100%">
						<tr class="form-field">
							...
							<td>
								<?php $accepting = $this->reservation_time_margin(); ?>
								<select id="booking-visit_time" type="text" name="booking[visit_time]" style="letter-spacing:2px"  required><?php foreach ($accepting as $min => $label) : ?>
									<?php if($label == $this->booking['visit_time']){ ?>
	 ...
	 *
	 */
	private function _postbox_show_name() {

		?>
			<div class="postbox">
				<h3><?php _e('名前の表示について') ?></h3>
				<div class="inside">
					<table class="form-table" style="width: 100%">
						<tr class="form-field">
							<th>
							...
							<td>
								<?php if($this->booking['show_name']=="表示する"){?>
									<input type="radio" name="booking[show_name]" value="表示する" checked>表示する
									<input type="radio" name="booking[show_name]" value="表示しない">表示しない
								<?php } else{?>								
									<input type="radio" name="booking[show_name]" value="表示する">表示する
									<input type="radio" name="booking[show_name]" value="表示しない" checked>表示しない
								<?php } ?>
							</td>
						</tr>
					</table>
	 ...
	 *
	 */
	private function _postbox_show_introducer() {

		?>
			<div class="postbox">
				<h3><?php _e('ご紹介者のお名前') ?></h3>
				<div class="inside">
					<table class="form-table" style="width: 100%">
						<tr class="form-field">
							<th>
							...
							<td>
								<input id="booking-introducer" type="text" name="booking[introducer]"  value="<?php echo esc_attr($this->booking['introducer']) ?>"/>
							</td>
						</tr>
					</table>
				</div>
